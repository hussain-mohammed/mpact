    ...
    <div class='auth-container vw-100 vh-100 position-relative'>
      <header>
        <div
          ...
          <div class='login-header-btn text-white cursor-pointer' :class="{ 'text-danger': !nextButtonValidation }"
      ...
      <section class='position-relative'>
        <div class='position-absolute mx-auto left-right-0 w-100 d-grid justify-content-center login-top'>
          <Login v-if='showLoginPage' @moveToNextPage='nextButtonValidation && moveToNextPage()'>
            <vue-tel-input v-model='phoneNumber' v-bind='telephoneProps' @validate='validatePhoneNumber($event)'>
            </vue-tel-input>
          </Login>
          <OTP v-if='showOTPPage' @hideOtpComponent='showOTPPage = !showOTPPage; showLoginPage = !showLoginPage;'
          ...
          <div class='login-footer width-404 text-center' v-show='!isInfoHide'>
            ...
            <div class='footer-2 cursor-pointer' @click='isInfoHide = !isInfoHide' v-show='!isInfoHide'>
              Learn more
            </div>
          </div>
